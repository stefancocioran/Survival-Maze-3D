...
void main()
{
    ...
    if (diffuse_light > 0)
    {
        ...
        specular_light = pow(specular_light, 10);
    }
    ...
    float atten = 1/ (0.1 * pow(d, 2));

    // TODO(student): Send color light output to fragment shader
    color = ambient_light + atten * (diffuse_light * material_kd + specular_light * material_ks) * object_color;
