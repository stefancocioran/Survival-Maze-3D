...
void Lab9::Update(float deltaTimeSeconds)
{
...

	{
		...
		modelMatrix = glm::rotate(modelMatrix, GetSceneCamera()->m_transform->GetRotationEulerRad().y, glm::vec3(0.f, 1.f, 0.f));
...
void Lab9::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	{
		glUniform1f(glGetUniformLocation(shader->program, "time"), -1.f);
	}
	...
	if (texture2)
	{
		...
		glUniform1i(glGetUniformLocation(shader->program, "use_texture_2"), 1);
	}
	else {
		glUniform1i(glGetUniformLocation(shader->program, "use_texture_2"), 0);
	}
